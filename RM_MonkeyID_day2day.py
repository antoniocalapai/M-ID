...
if makemodel:
    ...
    for m in df.animalID.unique():
        selected = random.sample(df[(df['animalID'] == m) & (df['date'] < sessions[training_sessions-1])].FileName.to_list(), TEST_N)
    ...
    n_samples = min(df[(df['animalID'] == m) & (df['date'] < sessions[training_sessions-1])]['animalID'].value_counts() - TEST_N)
    for m in df.animalID.unique():
        non_test_list = df[(df['animalID'] == m) & (df['test'] == 0) & (df['date'] < sessions[training_sessions-1])].FileName.to_list()
...
test_df = df[df['date'] > sessions[training_sessions-1]]

c = 0
for category in categories:
    all_elements = test_df[test_df['animalID'] == category].FileName.to_list()
...
test_run['failures'] = test_run['label'] != test_run['result']
test_run.to_csv('./test_run_DF_EvS.csv', sep=',')

# plot_df = test_run.groupby(['session', 'label']).apply(lambda x: x['failures'].sum()/len(x)).reset_index(name ='acc')
# sns.lineplot(x="session", y="acc", hue="label", data=plot_df)