...
model_name = 'MultiNet_Bal_ExtSetSize'
Nsessions = 6
makemodel = 1
makedf = 0
...
if makedf:
    ...
    for group in groups:
        ...
        n_samples = min(df[(df['predict'] == 0) & (df['group'] == group)]['animal'].value_counts())
        print(group + ' ' + str(n_samples))
        df.loc[df['group'] == group, 'CNN_Npics'] = n_samples
        df.loc[df['group'] == group, 'CNN_name'] = "{}{}{}".format(model_name, '_', group)
