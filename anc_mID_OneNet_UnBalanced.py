...
if makedf:
    ...
    for m in categories:
        n_samples = np.floor(min(df[(df['predict'] == 0) & (df['animal'] == m) & (df['predict'] == 0)]
                                 ['animal'].value_counts()) / 3)

        df.loc[df['animal'] == m, 'CNN_Npics'] = n_samples
        non_test_list = df[(df['animal'] == m) & (df['predict'] == 0)].pic_name.to_list()

        if len(non_test_list):
            selected = random.sample(non_test_list, int(n_samples))
