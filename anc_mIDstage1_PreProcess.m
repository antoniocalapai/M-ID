...
d = dir([dataframePath '*mID_stage1_Trials*']);
dd = zeros(length(d),1);
for j = 1:length(d)
...
for i = 1:length(datafiles)
    ...
    if ExtractSession == 1 || ExtractAllSessions == 1
        ...
        % save the dataframe
        writetable(T, string(dataframePath) + 'mID_stage1_TrialsDataFrame_' + string(yyyymmdd(datetime('today')))+ '.csv')
        disp('Data saved')
    end
end
%end