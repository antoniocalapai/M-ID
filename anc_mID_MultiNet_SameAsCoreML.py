...
model_name = 'MultiNet_SameAsCoreML'
Nsessions = 3
makemodel = 0
makedf = 0
...
if makedf:
    ...
    csv_file = "{}{}".format(DF_path, 'CoreML_TrainingSet.csv')
    train_set_df = pd.read_csv(csv_file, index_col=0, low_memory=False)

    for i in progressbar(train_set_df[train_set_df['train'] == 1]['pic_name']):
        if len(df[df['pic_name'] == i]) > 0:
            df.loc[df['pic_name'] == i, 'train'] = 1

    for group in groups:
        df.loc[df['group'] == group, 'CNN_Npics'] = sum(df[df['group'] == group]['train']) / 2
