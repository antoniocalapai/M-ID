...
# =============================================
# TODO Compare OneNet Balanced vs Unbalanced
...
models = ['OneNet_Balanced', 'OneNet_UnBalanced']
OneNet = pd.DataFrame()
for m in models:
...
sns.barplot(x="session", y="accuracy", hue="Model", data=OneNet, ci=95, ax=ax[0, 0])
sns.barplot(x="animal", y="accuracy", hue="Model", data=OneNet, ci=95, ax=ax[1, 0])
sns.barplot(x="hour", y="accuracy", hue="Model", data=OneNet, ci=95, ax=ax[2, 0])
...
sns.boxplot(x="Model", y="accuracy", hue="Model", whis=[0, 100], data=OneNet_stat, ax=ax[0, 1])
g = sns.stripplot(x="Model", y="accuracy", hue="Model", dodge=True, color='0.3',
                  size=4, linewidth=0, data=OneNet_stat, ax=ax[0, 1])

t, p = ranksums(OneNet_stat[OneNet_stat['Model'] == 'UnBalanced']['accuracy'],
                OneNet_stat[OneNet_stat['Model'] == 'Balanced']['accuracy'])
...
for idx, m in enumerate(models):
    ...
    OneNet_stat[m.split('_')[1]] = T.accuracy.values
    OneNet_stat['hour'] = T.hour

OneNet_stat['Balanced-Unbalanced'] = OneNet_stat[models[0].split('_')[1]] - OneNet_stat[models[1].split('_')[1]]
sns.histplot(OneNet_stat['Balanced-Unbalanced'], bins=20, ax=ax[1, 1])
ax[1, 1].set_xlim([-0.2, 0.2])
ax[1, 1].axvline(OneNet_stat['Balanced-Unbalanced'].mean(), color='red', linestyle='--', alpha=0.7)
t, p = ttest_1samp(OneNet_stat['Balanced-Unbalanced'], popmean=0)
...
OneNet_stat = OneNet_stat.reset_index(drop=True)
sns.regplot(x='hour', y='accuracy', data=OneNet_stat[OneNet_stat['Model'] == 'Balanced'], truncate=False, ax=ax[2,1])
sns.regplot(x='hour', y='accuracy', data=OneNet_stat[OneNet_stat['Model'] == 'UnBalanced'], truncate=False, ax=ax[2,1])
ax[2, 1].set_ylim([0.80, 1.01])

X = OneNet_stat[OneNet_stat['Model'] == 'Balanced']['hour']
Y = OneNet_stat[OneNet_stat['Model'] == 'Balanced']['accuracy']
R, p = stats.pearsonr(X, Y)

ax[2, 1].text(8, 0.85, "{}{}{}{}".format('pearson R = ', "%.2f"%R, ' ; p = ', "%.2f"%p),
              color='blue', fontsize=12, va="bottom", ha="left")

X = OneNet_stat[OneNet_stat['Model'] == 'UnBalanced']['hour']
Y = OneNet_stat[OneNet_stat['Model'] == 'UnBalanced']['accuracy']
R, p = stats.pearsonr(X, Y)
...
plot_name = 'OneNet_BalancedVsUnbalanced'
