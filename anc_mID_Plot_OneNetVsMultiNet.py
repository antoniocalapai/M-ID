...
# =============================================
models = ['OneNet_Balanced', 'MultiNet_Balanced']
palette = {models[0].split('_')[0]: "b", models[1].split('_')[0]: "g"}
OneNet = pd.DataFrame()
...
t, p = ranksums(OneNet_stat[OneNet_stat['Model'] == 'OneNet']['accuracy'],
                OneNet_stat[OneNet_stat['Model'] == 'MultiNet']['accuracy'])
...
OneNet_stat['OneNet-MultiNet'] = OneNet_stat[models[0].split('_')[0]] - OneNet_stat[models[1].split('_')[0]]
sns.histplot(OneNet_stat['OneNet-MultiNet'], bins=20, ax=ax[1, 1])
ax[1, 1].set_xlim([-0.2, 0.2])
ax[1, 1].axvline(OneNet_stat['OneNet-MultiNet'].mean(), color='red', linestyle='--', alpha=0.7)
t, p = ttest_1samp(OneNet_stat['OneNet-MultiNet'], popmean=0)
...
ax[1, 1].set_xlabel(xlabel='Accuracy\n[OneNet-MultiNet]', fontsize=labelFontSize)
...
OneNet_stat = OneNet_stat.reset_index(drop=True)
sns.regplot(x='hour', y='accuracy', data=OneNet_stat[OneNet_stat['Model'] == 'OneNet'], truncate=False, ax=ax[2,1])
sns.regplot(x='hour', y='accuracy', data=OneNet_stat[OneNet_stat['Model'] == 'MultiNet'], color="g",
            truncate=False, ax=ax[2,1])
ax[2, 1].set_ylim([0.80, 1.01])

X = OneNet_stat[OneNet_stat['Model'] == 'OneNet']['hour']
Y = OneNet_stat[OneNet_stat['Model'] == 'OneNet']['accuracy']
R, p = stats.pearsonr(X, Y)

ax[2, 1].text(8, 0.85, "{}{}{}{}".format('pearson R = ', "%.2f"%R, ' ; p = ', "%.2f"%p),
              color='blue', fontsize=12, va="bottom", ha="left")

X = OneNet_stat[OneNet_stat['Model'] == 'MultiNet']['hour']
Y = OneNet_stat[OneNet_stat['Model'] == 'MultiNet']['accuracy']
R, p = stats.pearsonr(X, Y)

ax[2, 1].text(8, 0.85, "{}{}{}{}".format('pearson R = ', "%.2f"%R, ' ; p = ', "%.2f"%p),
              color='green', fontsize=12, va="top", ha="left")
...
plot_name = 'OneNet_OneNetVsMultiNet'
