...
# =============================================
models = ['MultiNet_SameAsCoreML', 'MultiNet_UnBalanced']
palette = {models[0].split('_')[1]: "b", models[1].split('_')[1]: "g"}
DATA = pd.DataFrame()
...
for idx, m in enumerate(models):
    ...
    DATA_stat[m.split('_')[1]] = T.accuracy.values
    DATA_stat['hour'] = T.hour

DATA_stat['SameAsML-Balanced'] = DATA_stat[models[0].split('_')[1]] - DATA_stat[models[1].split('_')[1]]
sns.histplot(DATA_stat['SameAsML-Balanced'], bins=20, ax=ax[1, 1])
ax[1, 1].set_xlim([-0.2, 0.2])
ax[1, 1].axvline(DATA_stat['SameAsML-Balanced'].mean(), color='red', linestyle='--', alpha=0.7)
t, p = ttest_1samp(DATA_stat['SameAsML-Balanced'], popmean=0)
...
ax[1, 1].set_xlabel(xlabel='Accuracy\n[SameAsML-Balanced]', fontsize=labelFontSize)
...
DATA_stat = DATA_stat.reset_index(drop=True)
sns.regplot(x='hour', y='accuracy', data=DATA_stat[DATA_stat['Model'] == models[0].split('_')[1]],
            truncate=False, ax=ax[2,1])
sns.regplot(x='hour', y='accuracy', data=DATA_stat[DATA_stat['Model'] == models[1].split('_')[1]], color="g",
            truncate=False, ax=ax[2,1])
ax[2, 1].set_ylim([0.80, 1.01])

X = DATA_stat[DATA_stat['Model'] == models[0].split('_')[1]]['hour']
Y = DATA_stat[DATA_stat['Model'] == models[0].split('_')[1]]['accuracy']
R, p = stats.pearsonr(X, Y)

ax[2, 1].text(8, 0.85, "{}{}{}{}".format('pearson R = ', "%.2f"%R, ' ; p = ', "%.2f"%p),
              color='blue', fontsize=12, va="bottom", ha="left")

X = DATA_stat[DATA_stat['Model'] == models[1].split('_')[1]]['hour']
Y = DATA_stat[DATA_stat['Model'] == models[1].split('_')[1]]['accuracy']
R, p = stats.pearsonr(X, Y)

ax[2, 1].text(8, 0.85, "{}{}{}{}".format('pearson R = ', "%.2f"%R, ' ; p = ', "%.2f"%p),
              color='g', fontsize=12, va="top", ha="left")
...
plot_name = 'mID_SameAsMLVsMultiUnBalanced'
