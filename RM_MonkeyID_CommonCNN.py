...
if makemodel:
    ...
    csv_file = './img_DF_all.csv'
    df = pd.read_csv(csv_file, index_col=0, low_memory=False, sep=';')
    ...
    # ==================
    # Save the model
    model.save('RM_all.model')

# Load the existing model and image dataframe
else:
    model = keras.models.load_model("RM_all.model")
...
test_run = pd.DataFrame(columns=['image', 'session', 'label', 'result', 'confidence'])
categories = df.animalID.unique().tolist()
for category in categories:
    all_elements = df[(df['animalID'] == category) & (df['test'] == 1)].FileName.to_list()
