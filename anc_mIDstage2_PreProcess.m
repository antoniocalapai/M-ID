...
% anc_mIDv2_extract_DATA_Pics(1, 0) -> will process and extract the pictures and trial information only of new data files
...
d = dir([dataframePath '*mID_stage2_Trials*']);
dd = zeros(length(d),1);
for j = 1:length(d)
...
for i = 1:length(datafiles)
    ...
    if ExtractSession == 1 || ExtractAllSessions == 1
        ...
        data = MW_readFile(fileName, 'include', {'#announceMessage'}, ...
            'stepSize', 60000000 * 600, 'debugLevel', 0);
        ...
        params = {'TRIAL_', 'IO_midIdentifier', 'IO_midImage', 'IO_start'};
        ...
        for k = 1:length(trial.start)
            ...
            trial_trigger = data.value(data.time > trial.start(k) & data.time < trial.end(k) & data.event == 'IO_start');
            ...
            imageID = data.value(data.time > trial.start(k) & data.time < trial.end(k) & data.event == 'IO_midIdentifier');
            imageIDTime = data.time(data.time > trial.start(k) & data.time < trial.end(k) & data.event == 'IO_midIdentifier');
            
            % if the trial was triggered by the animal and not automatically
            if ~isempty(outcome)
                if strcmp(outcome{1}, 'hit') || strcmp(outcome{1}, 'noAnswer')
                    if ~strcmp(imageID, 'noCamera')
                        ...
                        % if the session belongs to the first phase of the experiment
                        if strcmp(pattern(5), 'MonkeyID')
                            ...
                            % if the session belongs to the second phase of the experiment
                        else
                            % store the label from the ML model
                            nRow.ML_label = string(lower(imageID));
                            % compute the roundtrip time
                            nRow.roundTrip = string(imageIDTime - imageTime);
                            % set the experiment as "testing"
                            nRow.experiment = "testing";
                        end
                        ...
                        % if the camera module produced an error
                    elseif strcmp(imageID, 'noCamera')
                        % send the information to the command window and assign nans
                        disp('Problems with picture at trial ' + string(k) + ' of ' + fileName(end-47:end));
                        nRow.ML_label = 'noCamera';
                        nRow.time = 'NA';
                        nRow.roundTrip = 'NA';
                        nRow.time_of_day = 'NA';
                    end
        ...
        % append the extracted data to the existing dataframe    
        T = vertcat(T,DATA);
        
        % save the dataframe
        writetable(T, string(dataframePath) + 'mID_stage2_TrialsDataFrame_' + string(yyyymmdd(datetime('today')))+ '.csv')
        disp('Data saved')
    end
end
%end